...
export default function RateCalculator({ tours, rateCards, clients = [], agents = [], bookings = [], hotels: initHotels = [], destinations: initDests = [] }: Props) {
  ...
  const [options, setOptions] = useState([
    ...
    { pax: 6, markup: 10, rateCharged: 0 },
    { pax: 8, markup: 10, rateCharged: 0 },
  ]);
  ...
  const optionResults = options.map(opt => {
    ...
    const rateCharged = opt.rateCharged || markedUp;
    const profit = rateCharged - markedUp;
    return { ...opt, perPersonBase, markedUp, profit };
  });
