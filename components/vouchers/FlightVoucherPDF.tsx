// components/vouchers/FlightVoucherPDF.tsx
...
const s = StyleSheet.create({
  ...
  // FLIGHT VOUCHER spaced title
  titleWrap:  { alignItems: 'center', marginBottom: 4 },
  flightLbl:  { fontSize: 10, fontFamily: 'Helvetica-Bold', color: B, letterSpacing: 4, textDecoration: 'underline' },
  voucherNo:  { fontSize: 14, fontFamily: 'Helvetica-Bold', color: B, marginTop: 3, marginBottom: 14 },
    ...
    <Document title={`Flight Voucher ${voucher.voucherNo}`}>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.titleWrap}>
          <Text style={s.flightLbl}>F L I G H T  V O U C H E R</Text>
          <Text style={s.voucherNo}>Voucher No: {voucher.voucherNo}</Text>
        </View>

        <Text style={s.date}>Date: {fmtDate(voucher.issuedDate)}</Text>
        {/* Booking status badge */}
        {voucher.bookingStatus && voucher.bookingStatus !== 'book' && (
          <View style={{ alignSelf: 'flex-end', marginTop: -22, marginBottom: 8 }}>
            <Text style={[s.badge, {
        ...
        <View style={s.fieldRow}>
          <Text style={s.fieldLabel}>Flight Name: </Text>
          <Text style={s.fieldValue}>{voucher.flightName || '—'}</Text>
        </View>

        {/* Schedule */}
        <View style={s.fieldRow}>
          <Text style={s.fieldLabel}>Schedule   : </Text>
          <Text style={s.fieldValue}>{voucher.flightSchedule || '—'}</Text>
        </View>
        ...
        <View style={s.checkRow}>
          <Text style={s.checkLabel}>Departure:</Text>
          <Text style={s.checkValue}>{fmtDate(voucher.departureDate)}</Text>
        </View>
        <View style={s.checkRow}>
          <Text style={s.checkLabel}>Return:</Text>
          <Text style={s.checkValue}>{fmtDate(voucher.returnDate)}</Text>
        </View>
        <View style={s.checkRow}>
          <Text style={s.checkLabel}>Number of Days:</Text>
          <Text style={s.checkValue}>{voucher.numDays ?? 0}</Text>
        </View>
